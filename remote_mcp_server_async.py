...
@mcp.tool()
async def list_expenses(start_date: str, end_date: str, category: Optional[str] = None):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as c:
            ...
            expenses = [dict(zip(cols, r)) for r in await cur.fetchall()]
...
async def summarize(start_date: str, end_date: str, category: Optional[str] = None):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as c:
            ...
            cur = await c.execute(query, params)
            cols = [d[0] for d in cur.description]
            summary = [dict(zip(cols, r)) for r in await cur.fetchall()]
...
@mcp.tool()
async def delete_expense(id: Optional[int] = None, ids: Optional[List[int]] = None, 
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            ...
            if ids is not None and len(ids) > 0:
                ...
                await c.execute(delete_query, ids)
                await c.commit()
                return {
                    "status": "ok", 
            ...
            if conditions:
                ...
                cur = await c.execute(count_query, params)
                count = await cur.fetchone()[0]
                ...
                await c.execute(delete_query, params)
                await c.commit()
                return {
                    "status": "ok",
                    "message": f"{count} expense(s) deleted successfully",
...
@mcp.tool()
async def update_expense(id: int, date: Optional[str] = None, amount: Optional[float] = None, 
...
@mcp.tool()
async def get_monthly_trends(year: int, category: Optional[str] = None):
...
@mcp.tool()
async def get_top_expenses(start_date: str, end_date: str, limit: int = 10):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as c:
            query = """
                ...
                LIMIT ?
            """
            cur = await c.execute(query, (start_date, end_date, limit))
            cols = [d[0] for d in cur.description]
            expenses = [dict(zip(cols, r)) for r in await cur.fetchall()]
...
@mcp.tool()
async def get_category_breakdown(start_date: str, end_date: str):
...
async def get_budget_status(year_month: str):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as c:
            # Get all budgets
            cur = await c.execute("SELECT category, monthly_limit, alert_threshold FROM budgets")
            ...
            for category, limit, threshold in budgets:
                ...
                spent, count = await cur.fetchone()
                percentage = (spent / limit) * 100 if limit > 0 else 0
                remaining = limit - spent
...
async def list_budgets():
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            cur = await c.execute("SELECT category, monthly_limit, alert_threshold FROM budgets ORDER BY category")
            budgets = []
            for row in await cur.fetchall():
                budgets.append({
                    "category": row[0],
...
async def delete_budget(category: str):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            cur = await c.execute("SELECT category FROM budgets WHERE category = ?", (category,))
            if not await cur.fetchone():
                return {"status": "error", "message": f"No budget found for category '{category}'"}
            
            await c.execute("DELETE FROM budgets WHERE category = ?", (category,))
            await c.commit()
...
@mcp.tool()
async def add_recurring_expense(amount: float, category: str, frequency: str, start_date: str,
...
@mcp.tool()
async def list_recurring_expenses(active_only: bool = True):
...
@mcp.tool()
async def deactivate_recurring_expense(id: int):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            cur = await c.execute("SELECT id FROM recurring_expenses WHERE id = ?", (id,))
            if not await cur.fetchone():
                return {"status": "error", "message": f"Recurring expense with id {id} not found"}
            
            await c.execute("UPDATE recurring_expenses SET active = 0 WHERE id = ?", (id,))
            await c.commit()
...
@mcp.tool()
async def export_to_file(start_date: str, end_date: str, filename: str, format: str = "csv"):
